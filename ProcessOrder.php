...
if(isset($_POST['UserName']))
{
	...
	if(isset($_COOKIE['ShoppingCart']) && $_COOKIE['ShoppingCart'] != "")
	{
		...
		$statement1 = $dbh->prepare('SELECT CustomerID FROM Customers WHERE UserName = ?  ');
		...
		if($row = $statement1->fetch(PDO::FETCH_ASSOC))
		{
			...
			$statement2 = $dbh->prepare('INSERT INTO Orders ( CustomerID) VALUES (?); ');
			
			$statement2->bindValue(1,$customerID);
			$statement2->execute();			
